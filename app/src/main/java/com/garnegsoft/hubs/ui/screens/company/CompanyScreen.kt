...
fun CompanyScreen(
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(it)) {
            val tabs = remember(companyProfile) {
                var map: Map<String, @Composable () -> Unit> = mapOf(
                    "Профиль" to {
                        CompanyProfile(scrollState = profilePageScrollState, viewModel = viewModel)
                        LaunchedEffect(Unit) {
                            if (whoIs == null) {
                                launch(Dispatchers.IO) {
            ...
            AnimatedVisibility(
                ...
                HabrScrollableTabRow(
                    ...
                    when {
                        ...
                        title.startsWith("Сотрудники") -> {
                            ScrollUpMethods.scrollLazyList(employeesLazyListState)
                        }
                    }
                }
            }

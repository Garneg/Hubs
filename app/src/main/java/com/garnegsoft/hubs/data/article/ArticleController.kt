...
import com.garnegsoft.hubs.data.article.offline.HubsList
...
class ArticleController {
    ...
    companion object {
        ...
        ): com.garnegsoft.hubs.data.article.Article? {
        ...
        private fun get(
            ...
            return raw?.let {
                ...
                com.garnegsoft.hubs.data.article.Article(
                    ...
                    metadata = if (it.metadata != null) com.garnegsoft.hubs.data.article.Article.Metadata(
                    ...
                    contentHtml = it.textHtml,
                    translationData = com.garnegsoft.hubs.data.article.Article.TranslationData(
                        isTranslation = it.postLabels?.find { it.type == "translation" } != null,
                    ...
                    polls = it.polls.map {
                        com.garnegsoft.hubs.data.article.Article.Poll(
                            id = it.id.toInt(),
                            timeFinish = it.timeElapsed,
                            answersType = when(it.answersType){
                                "checkbox" -> com.garnegsoft.hubs.data.article.Article.Poll.PollType.Checkbox
                                else -> com.garnegsoft.hubs.data.article.Article.Poll.PollType.Radio
                            },
                            votesCount = it.votesCount,
                            passCount = it.passCount,
                            title = it.textHtml,
                            relatedData = it.relatedData?.let {
                                com.garnegsoft.hubs.data.article.Article.Poll.RelatedData(it.canVote)
                            },
                            variants = it.variants.map {
                                com.garnegsoft.hubs.data.article.Article.Poll.Variant(
                                    id = it.id.toInt(),
                                    text = it.textHtml,
                                    votesCount = it.votesCount,
                                    percent = it.percent,
